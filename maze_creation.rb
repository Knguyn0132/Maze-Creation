...
class GameWindow < Gosu::Window
  ...
  def search(cell_x ,cell_y)
    ...
    if (cell_x == ((MAP_WIDTH / CELL_DIM) - 1)) #check if it the most east cells
      #if (ARGV.length > 0) # debug 
      #holds the command line arguments passsed
        puts "End of one path x: " + cell_x.to_s + " y: " + cell_y.to_s
      #end
      [[cell_x,cell_y]]  # We are at the east wall - exit
    else

      north_path = nil  #path directions
      west_path = nil 
      east_path = nil
      south_path = nil
      ...
      if (north_path != nil)
        ...
        path = east_path
      elsif (west_path != nil) #pick the path to go
        path = west_path
      end
      ...
      if (path != nil)
        #if (ARGV.length > 0) # debug
          puts "Added x: " + cell_x.to_s + " y: " + cell_y.to_s #print cells being added
        #end
        [[cell_x,cell_y]].concat(path) #combine array
        #2-D array
        #[[cell_x, cell_y], path_element1, path_element2, ..., path_elementN]
